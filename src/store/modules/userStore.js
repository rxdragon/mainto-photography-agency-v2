  ...
  getUserInfo ({ commit }) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const info = saveInfo || await UserAction.getInfo()
        SessionTool.setUserInfo(info.msg)
        commit('SET_USERINFO', info.msg)
  ...
  getUserPermission () {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        resetRouter()
        accessRoutes.forEach(roleItem => {
          router.addRoute(roleItem)
        })
        resolve()
      } catch (error) {
        router.push({
